    ...
    if(isset($_SESSION['id_users']) && isset($_SESSION['login'])){
        if(isset($_POST['old_password']) && isset($_POST['new_password'])){
            ...
            if(empty($old_password)){
                header("Location: change_pass.php?error=Wymagane jest stare hasło - podaj je");
                exit();
            }
            else if(empty($new_password)){
                header("Location: change_pass.php?error=Wymagane jest nowe hasło - podaj je");
                exit();
            }
            else{
                ...
                if($temp==$_POST['old_password']){
                    $sql2 = "UPDATE users SET password = '".$new_password."' WHERE ID_users LIKE '".$_SESSION['id_users']."';";
                    $result = mysqli_query($conn, $sql2);
                    header("Location: change_pass.php?fine=Hasło zostało zmienione!");
                    exit();
                }
                else{
                    header("Location: change_pass.php?error=Błędne stare hasło!");
                    exit();
                }
            }
        }
...
<body>
    <div class="block">
        ...
        <form id="search_form" action="change_pass.php" method="POST">
            <h2>Zmiana hasła</h2>
